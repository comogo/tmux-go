...
// Includes asserts that the specified iterable contains the specified element.
//
//    assert.Includes(t, someSlice, "Foo", "But ["Foo", "Bar"] does include 'Foo'")
//
// Returns whether the assertion was successful (true) or not (false).
func Includes(t TestingT, list interface{}, element interface{}, msgAndArgs ...interface{}) bool {
	ok, found := includeElement(list, element)
	if !ok {
		return Fail(t, fmt.Sprintf("\"%s\" could not be applied builtin len()", list), msgAndArgs...)
	}
	if !found {
		Fail(t, fmt.Sprintf("%v should include %s", list, element), msgAndArgs...)
	}
	return found
}

// NotIncludes asserts that the specified iterable does NOT contain the specified element.
//
//    assert.NotIncludes(t, someSlice, "Foo", "But '["Foo", "Bar"]' does NOT include 'Baz'")
//
// Returns whether the assertion was successful (true) or not (false).
func NotIncludes(t TestingT, list interface{}, element interface{}, msgAndArgs ...interface{}) bool {
	ok, found := includeElement(list, element)
	if !ok {
		return Fail(t, fmt.Sprintf("\"%s\" could not be applied builtin len()", list), msgAndArgs...)
	}
	if found {
		Fail(t, fmt.Sprintf("%v should not include %s", list, element), msgAndArgs...)
	}
	return !found
}
